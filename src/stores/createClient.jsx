...
export default function createClient([actions]) {
  async function send(method, url, data, resKey, idx, url_root = API_ROOT) {
    const headers = {},
      opts = { method, headers };
  ...
  const Locations = {
    get: (id) =>
      send("get", `/latestAll?sensor_id=${id}`, undefined, "results"),
  };
  ...
  const Providers = {
    getAll: () => send("get", `/v3/providers?limit=1000`, undefined, "results"),
  };
  ...
  const Measurements = {
    get: (params) => {
      ...
      const datetimeStart = dayjs(dateFrom).utcOffset(offset, true).format();
      const datetimeEnd = dayjs(dateTo).utcOffset(offset, true).format();
  ...
  const Downloads = {
    get: (props) => {
      ...
      const datetimeStart = dayjs(props.dateFrom)
        ...
        .format();
      const datetimeEnd = dayjs(props.dateTo).utcOffset(offset, true).format();
      let parameterParams = "";
      if (props.parameters) {
